 ...
 * @file servo.c
 * @date October 2016
 * @brief Sets PWM duty cycle based 
 * 
 * Determines required servo PWM duty cycle based on input angle.
...
//char turnFLAG = 0;
 ...
 //! Delay variables.
    /*!
     Variables for MSB and LSB of high an low time.
    */
...
/** Set interrupt delays. Sets the high and low time
 * delays to generate a suitable servo control PWM.
 */
...
/** Initialise servo delay interrupts.
 */
...
/** Initialise CCP2 for an initial delay.
 */
...
/** Function called in PrimaryInterfaceOutput.
 */
void servoOutput(int degrees){
    degrees2pulse_LOW(degrees);
    degrees2pulse_HIGH(degrees);
}
/** Choose low delay based on angle. Sets suitable PWM delays based
 * on angle input. Delays for servo have be calibrated in 45 deg
 * increments.
 */
...
/** Choose high delay based on angle. Sets suitable PWM delays based
 * on angle input. Delays for servo have be calibrated in 45 deg
 * increments.
 */
